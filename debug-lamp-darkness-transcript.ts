...
/**
 * Debug script to analyze lamp and darkness transcript failures
 */
...
// Load the transcript
const transcript = JSON.parse(fs.readFileSync('.kiro/transcripts/critical/04-lamp-darkness.json', 'utf-8'));

console.log('='.repeat(80));
console.log('LAMP AND DARKNESS TRANSCRIPT DEBUG');
console.log('='.repeat(80));
console.log();
...
for (let i = 0; i < transcript.entries.length; i++) {
  ...
  console.log(`\nExpected (${normExpected.length} chars):`);
  console.log(entry.expectedOutput);
  console.log(`\nActual (${normActual.length} chars):`);
